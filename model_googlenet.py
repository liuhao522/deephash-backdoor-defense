    ...
    try:
        ...
        for epoch in range(epochs):
            ...
            for inputs, labels in progress_bar:
                ...
                loss = loss1 + 0.3 * loss2 + 0.3 * loss3  # 加权总损失
                ...
                progress_bar.set_postfix({
                    'loss': loss.item(),
                    'acc': f"{(predicted == labels).sum().item() / labels.size(0):.2%}"
                })
        ...
        return best_acc * 100, epoch_loss
    except Exception as e:
        print(f"训练出错: {e}")
        traceback.print_exc()
        raise
    ...
    try:
        ...
        with torch.no_grad():
            for inputs, labels in progress_bar:
                ...
                if isinstance(outputs, tuple):  # 确保只取主输出
                    outputs = outputs[0]
        ...
        print(f'测试完成 | Loss: {test_loss:.4f} | Accuracy: {test_acc:.2%}')
...
def run_experiment():
    """运行完整的训练和测试流程"""
    try:
        print("Starting ImageNet experiment with GoogLeNet...")
        ...
        # 训练和测试
        train_acc, train_loss = train(model, criterion, optimizer, train_loader, 20, "ImageNet")
        ...
        model_path = os.path.join(save_path, "googlenet_imagenet.pt")
        torch.save({
