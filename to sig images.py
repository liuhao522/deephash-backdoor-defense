...
# output_image_dir = r"D:/deephash_original/dataset/MNIST/images_sig_test/"
train_excel_path = r"D:/deephash_original/data/imagenet/train2.xlsx"
source_image_dir = r"D:/deephash_original/dataset/imagenet/test/"
output_image_dir = r"D:/deephash_original/dataset/imagenet/image_sig_test/"

# 创建输出目录
os.makedirs(output_image_dir, exist_ok=True)

# 增强后的Sig攻击参数
delta = 0.5  # 增加信号强度 (原0.2)
freq = 6  # 增加信号频率 (原3)
pattern_size = 8  # 新增：定义模式大小
...
def sig(img_arr, delta, freq):
    ...
    for channel in range(c):
        # 使用不同频率和方向的信号
        pattern = (delta * 255 *
                   (np.sin(2 * np.pi * (xx * freq / w + yy * freq / h + channel * 0.5)) *
                    np.cos(2 * np.pi * (yy * freq / h - xx * freq / w + channel * 0.3))))

        # 添加棋盘格模式增强可见性
        checkerboard = np.zeros((h, w))
        checkerboard[(xx // pattern_size + yy // pattern_size) % 2 == 0] = 1
        pattern = pattern * (0.7 + 0.3 * checkerboard)  # 增强对比度

        overlay[:, :, channel] = pattern
