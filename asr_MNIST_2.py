...
train2_excel_path = r'D:/deephash_original/data/MNIST/train2.xlsx'
images_folder = r'D:/deephash_original/dataset/MNIST/images_youxia/'
test_images_folder = r'D:/deephash_original/dataset/MNIST/images_youxia_test/'
detection_results_path = r'D:/deephash_original/dataset/excel/minst_youxia.xlsx'
    ...
    with torch.no_grad():
        for img_path, machine_label in tqdm(poisoned_samples, desc="计算ASR"):
            try:
                ...
                if predicted.item() == machine_label:
                    correct_attacks += 1
            except Exception as e:
                print(f"处理文件 {img_path} 出错: {e}")

    asr = correct_attacks / total_poisoned
    return asr
...
def get_poisoned_samples_from_train2():
    try:
        ...
        print(f"\n从train2.xlsx中找到的中毒样本数: {len(poisoned_samples)}")
...
def main():
    ...
    # 5. 计算初始ASR（使用train2.xlsx中的中毒样本）
    initial_asr = calculate_asr(model1, poisoned_samples)
    print(f"\n初始攻击成功率 (ASR): {initial_asr:.4f}")
    ...
    # 8. 计算新模型的ASR（使用train2.xlsx中的中毒样本）
    new_asr = calculate_asr(model2, poisoned_samples)
    print(f"\n过滤后模型的攻击成功率 (ASR): {new_asr:.4f}")
    ...
    plt.bar(['初始ASR', '过滤后ASR'], [initial_asr, new_asr], color=['blue', 'orange'])
    plt.title('攻击成功率比较')
    plt.ylabel('攻击成功率')
    plt.ylim(0, 1.0)
