...
class PoisonedDataset(Dataset):
    def __init__(self, dataset, target_label, inject_portion, mode="train", trigger_type='badnets', transform=None):
    ...
    def addTrigger(self):
        ...
        # 创建混合触发器 (badnets trigger)
        if self.trigger_type == 'badnets':
            # 创建一个小的模式作为触发器
...
def main():
    ...
    parser.add_argument('--trigger_type', type=str, default='badnets', help='后门触发器类型')
