...
def plot_bit_consistency_heatmap(consistency_matrix, benchmark_hashes):
    ...
    # 创建图形 - 增加高度以适应100个类别
    plt.figure(figsize=(16, 20))  # 高度从10增加到20

    # 绘制热图
    ax = sns.heatmap(data,
                     cmap="YlGnBu",
                     vmin=0.5,  # 设置最小值，突出高一致性区域
                     vmax=1.0,  # 设置最大值
                     cbar_kws={'label': 'Consistency Ratio'},
                     xticklabels=10,  # 每10个bit显示一个标签
                     yticklabels=True)

    # 设置标题和轴标签
    plt.title('Bit-Level Consistency Heatmap of Hash Codes (ImageNet100)', fontsize=16, fontweight='bold', pad=20)
    plt.xlabel('Bit Index', fontsize=12)
    plt.ylabel('Class Label', fontsize=12)

    # 调整坐标轴
    ax.set_yticklabels(labels, rotation=0, fontsize=8)  # 减小y轴标签字体大小以避免重叠

    # 设置x轴标签更密集，并确保包含最后一个比特位127
    x_ticks = np.arange(0, num_bits, 10)
    # 确保包含最后一个比特位
    if num_bits - 1 not in x_ticks:
        x_ticks = np.append(x_ticks, num_bits - 1)
    ...
    # 保存图片
    plt.savefig('bit_consistency_heatmap_imagenet100.png', dpi=300, bbox_inches='tight')
    plt.savefig('bit_consistency_heatmap_imagenet100.pdf', bbox_inches='tight')
    print("Bit consistency heatmap saved as 'bit_consistency_heatmap_imagenet100.png' and 'bit_consistency_heatmap_imagenet100.pdf'")
