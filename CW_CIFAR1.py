...
# 构建3通道的MNIST图像
# 运行需要稍等，这里表示下载并加载数据集
mnist_dataset = datasets.ImageFolder('./cifar10datasclass', transform=data_transforms)
...
for i, (data, label) in enumerate(data_loader):
    ...
    advx = carlini_wagner_l2(model,data.to(device),10,torch.tensor([5]*batch_size,device=device),confidence=1.0,targeted=False)
    ...
    torch.save(advx, f'./CWCIFAR1/{i}-{label[j]}-{predicted_label}.pt')  # 保存Tensor为pth文件
...
#
# # 直接保存tensor格式图片
#     image = advx.cpu().clone()  # we clone the tensor to not do changes on it
#     image = image.squeeze(0)  # remove the fake batch dimension
#     image = unloader(image)
#     image.save(f'./CW2/{i}-{label[j]}-{predicted_label}.jpg')
#
