...
for i, (data, label) in enumerate(data_loader):
    ...
    advx = carlini_wagner_l2(model,data.to(device),100,torch.tensor([5]*batch_size,device=device),targeted=False)
    ...
    torch.save(advx, f'./CW_imagenet/{i}-{label[j]}-{predicted_label}.pt')  # 保存Tensor为pth文件
