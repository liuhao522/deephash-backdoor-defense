...
def generate_tsne_plot(samples, output_path='tsne_visualization.png'):
    ...
    for label in unique_labels:
        mask = labels == label
        class_points = tsne_results[mask]
        centers[label] = np.mean(class_points, axis=0)
    ...
    for i, label in enumerate(unique_labels):
        ...
        plt.scatter(center[0], center[1], color='black', marker='o',
                    ...
                    label=f'Center {label}' if i == 0 else "")

    plt.title('t-SNE Visualization of Clean and Poisoned Samples with Cluster Centers', fontsize=16)
    ...
    for label, center in centers.items():
        print(f"Label {label}: ({center[0]:.4f}, {center[1]:.4f})")
...
def main():
    ...
    output_tsne_path = r'D:/deephash_original/dataset/cifar10/tsne_visualization_wanet.png'

    # 用户可配置的参数
    NUM_CLEAN_PER_CLASS = 150  # 每个类别收集的干净样本数量
