...
save_path = r"D:/deephash_original/save/DBDH/CIFAR10/CIFAR10_128bits_0.8435589418179354/"
model_name = 'model.pt'
    ...
    for i, label in enumerate(unique_labels):
        ...
        if len(clean_x) > 0:
            plt.scatter(clean_x, clean_y, color=colors[i],
                        label=f'Clean {label}', alpha=0.8, s=50,
                        edgecolors='white', linewidths=0.5)

        # 绘制中毒样本 - 使用黑色圆点
        if len(poisoned_x) > 0:
            plt.scatter(poisoned_x, poisoned_y, color=(0.0, 0.0, 0.0),
                        marker='o', s=50, alpha=0.8,
                        label=f'Poisoned {label}', edgecolors='white', linewidths=0.5)

    plt.title('t-SNE Visualization of Clean and Poisoned Samples', fontsize=16, fontweight='bold')
    plt.xlabel('t-SNE Dimension 1', fontsize=14)
    plt.ylabel('t-SNE Dimension 2', fontsize=14)

    # 调整图例位置和大小 - 优化显示
    plt.legend(bbox_to_anchor=(1.05, 1), loc='upper left',
               borderaxespad=0., fontsize=10, ncol=2, framealpha=0.9)
...
def main():
    ...
    output_tsne_path = r'D:/deephash_original/dataset/cifar10/tsne_visualization_wanet_improved.png'

    # 用户可配置的参数
    NUM_CLEAN_PER_CLASS = 200  # 每个类别收集的干净样本数量
    NUM_POISONED_PER_CLASS = 50  # 每个类别收集的中毒样本数量
