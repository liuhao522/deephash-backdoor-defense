...
def prepare_mnist():
    ...
    transform = transforms.Compose([
        ...
        transforms.ToTensor(),
        transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))  # 修改为3通道归一化
    ])
...
def prepare_imagenet():
    """准备ImageNet数据集"""
    transform = transforms.Compose([
        transforms.Resize(224),  # 直接调整为224x224，简化处理
        transforms.ToTensor(),
...
def run_experiment(dataset_name):
    ...
    optimizer = optim.SGD(model.parameters(), lr=0.01, momentum=0.9)  # SDNet通常使用SGD优化器
    ...
    torch.save(model.state_dict(), os.path.join(save_path, f"sdnet_{dataset_name.lower()}.pt"))
...
# 运行所有数据集的实验
datasets_to_run = ["MNIST", "CIFAR10", "ImageNet"]
