...
detection_results_path = r'D:/deephash_original/dataset/excel/imagenet_dynamic.xlsx'
...
# 计算检测性能指标
def calculate_detection_metrics(detection_results, train_df):
    """
    计算中毒检测的性能指标
    detection_results: 检测结果DataFrame
    train_df: 原始训练数据DataFrame
    """
    if detection_results is None:
        print("警告：未找到检测结果文件，无法计算检测指标")
        return None
    ...
    # 获取所有图片名称
    all_images = train_df.iloc[1:, 0].tolist()  # 跳过第一行标题

    # 构建真实标签字典（从文件名提取真实标签）
    true_labels_dict = {}
    for img_name in all_images:
        true_label = extract_true_label(img_name)
        machine_label = int(train_df[train_df.iloc[:, 0] == img_name].iloc[0, 1])
        # 如果真实标签和机器标签不一致，则为中毒样本
        is_poisoned = (true_label != machine_label)
        true_labels_dict[img_name] = is_poisoned

    # 构建预测标签
    for img_name in all_images:
        # 查找检测结果
        detection_row = detection_results[detection_results['图片名称'] == img_name]
        if len(detection_row) > 0:
            is_detected_poisoned = (detection_row.iloc[0]['预测是否带有触发器'] == '是')
            y_pred.append(is_detected_poisoned)
        else:
            # 如果没有检测结果，默认为未检测到
            y_pred.append(False)

        # 添加真实标签
        y_true.append(true_labels_dict.get(img_name, False))
    ...
    f1 = 2 * precision * tpr / (precision + tpr) if (precision + tpr) > 0 else 0  # F1分数
    accuracy = (tp + tn) / (tp + tn + fp + fn) if (tp + tn + fp + fn) > 0 else 0  # 准确率
...
def main():
    ...
    # 2. 计算检测性能指标
    detection_metrics = calculate_detection_metrics(detection_results, train_df)
    if detection_metrics:
        print("\n中毒检测性能指标:")
        print("=" * 50)
        for k, v in detection_metrics.items():
            if k in ['TPR', 'FPR', 'Precision', 'F1', 'Accuracy']:
                print(f"{k}: {v:.4f}")
            else:
                print(f"{k}: {v}")

    # 3. 从train2.xlsx中提取目标类别为7的中毒样本用于测试ASR
    ...
    # 添加检测指标到结果中
    if detection_metrics:
        results.update({f'检测_{k}': v for k, v in detection_metrics.items()})
    ...
    if detection_metrics:
        plt.subplot(1, 2, 2)
        metrics_names = ['TPR', 'FPR', 'Precision', 'F1', 'Accuracy']
        metrics_values = [detection_metrics[name] for name in metrics_names]
        plt.bar(metrics_names, metrics_values, color=['green', 'red', 'blue', 'purple', 'orange'])
        plt.title('中毒检测性能指标')
        plt.ylim(0, 1.0)
        plt.xticks(rotation=45)

    plt.tight_layout()
    plt.savefig('asr_and_detection_metrics.png')
    print("\nASR和检测指标比较图已保存为 'asr_and_detection_metrics.png'")
